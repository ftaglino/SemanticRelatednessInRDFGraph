...
public class SPARQLQueryCollector {
	...
	public static Vector<Node> nodesByPattern(SPARQLConnector sc, Object knowledgeSourceRef, String graph, PathPattern pattern) {
		...
		for(int i=0; i<pattern.getTriples().size(); i++) {
			...
			if(s.isVariable()) {
				// ... the variable is set as the variable to SELECT, that is the result
				varToSelect = s.toString();
...
//			else whereClause = whereClause + " \n ";
		...
		String queryString = "SELECT "+varToSelect+" "+graphFilter+" WHERE "
					+ "{"
					+ 	whereClause
					+	filterConstraints
					+ "}";				
	 ...
	 * @return
	 */
	public static int countNodesByPattern(SPARQLConnector sc, Object knowledgeSourceRef, String graph, PathPattern pattern, String distinct) {
		int result = 0;
...
//			else whereClause = whereClause + " \n ";
		...
		String selectVars = " "+varToSelect+" ";
		
		// build a string representing all the filters
		String filterConstraints = pattern.filtersToString();
		String graphFilter = ((graph!=null) && (graph.length()!=0)) ? " FROM <"+graph+"> " : " ";  		
		
		String queryString = "SELECT (COUNT( "+distinct+" "+selectVars+" ) AS ?count) "+graphFilter+" WHERE "
					+ "{"
					+ 	whereClause
					+	filterConstraints
					+ "}";		
	...
	public static Vector<Path> pathsByPathPattern(SPARQLConnector sc, Object knowledgeSourceRef, String graph, PathPattern pattern) {
		...
		// collect what to SELECT
		Set<String> varsToSelect = new HashSet<String>();   
		...
		for(int i=0; i<pattern.getTriples().size(); i++) {
			...
			// if the node is a variable of which the value is requested as result then ...
			if(s.isVariable()) {
				// ... the variable is added as a variable to SELECT
				varsToSelect.add(s.toString());
			...
			// if the node is a variable of which the value is requested as result then ...
			if(p.isVariable()) {
				// ... the variable is set as a variable to SELECT
				varsToSelect.add(p.toString());
				// ... is added as subject of a triple of the WHERE clause
				whereClause = whereClause + " "+p.toString()+" ";
			}
			else whereClause = whereClause + " <"+p.toString()+"> ";
...
//			else whereClause = whereClause + " \n ";
		...
		String selectVars = "";
		for(String v:varsToSelect)
			selectVars = selectVars + " "+v+" ";
		...
		for(int i=0; i<query_results.size(); i++) {
			...
			for(Triple t:pattern.getTriples()) {
				...
				if(t.getSubject().toString().startsWith("?"))
					s_result = query_results.elementAt(i).get(t.getSubject().toString().substring(1)).asNode();
				else s_result = t.getSubject();
				
				if(t.getObject().toString().startsWith("?"))
					o_result = query_results.elementAt(i).get(t.getObject().toString().substring(1)).asNode();
				else o_result = t.getObject();
					
				if(t.getPredicate().toString().startsWith("?"))
					p_result = query_results.elementAt(i).get(t.getPredicate().toString().substring(1)).asNode();
