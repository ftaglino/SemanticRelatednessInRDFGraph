 ...
 *
 */
public class IC_Simple extends Abstract_IC {
	private static IC_Simple instance = null;
	
	protected IC_Simple(KnowledgeBase kb) {
		super(kb);
	}
	
	public synchronized static IC_Simple getInstance(KnowledgeBase kb){
    	if (instance == null){
    		instance = new IC_Simple(kb);
    	}
    	return instance;
    }
	...
	public double max_ic() {
		double result = 0;
		result = -Math.log(1/((double)this.getKb().countAllTriples()));
		return result;
	}
}
